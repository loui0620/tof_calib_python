...
n = 100 # Maximum iteration number
a1, b1, c1 = 1, 3, 2  # ground truth
h = np.linspace(0, 1, n)  # 产生包含噪声的数据
y = [np.exp(a1 * i ** 2 + b1 * i + c1) + random.gauss(0, 4) for i in h]
y = mat(y)  # convert to matrix format


def Func(abc, input):  # 需要拟合的函数，abc是包含三个参数的一个矩阵[[a],[b],[c]]
    a = abc[0, 0]
    b = abc[1, 0]
    c = abc[2, 0]
    return np.exp(a * input ** 2 + b * input + c)

def Deriv(abc, iput, n):  # Obtain derivative
    x1 = abc.copy()
    x2 = abc.copy()
    x1[n, 0] -= 0.000001
    x2[n, 0] += 0.000001
...
fx = mat(np.zeros((n, 1)))  # f(x)  100*1  error func
fx_tmp = mat(np.zeros((n, 1)))
xk = mat([[0.8], [2.7], [1.5]])  # Initialize parameter
last_mse = 0
step = 0
u, v = 1, 2
conve = 100
